...
class Server:
    ...
    def send_model(self, client_socket):
        ...
        if self.iteration_number != 0:
            ...
            data = {"STOP_FLAG": self.stop_flag, "WEIGHTS": weights, "ITERATION_FLAG": self.all_timestamps_finished, "NUM_TIMESTAMPS": self.num_timestamps, "ORG_ID": ORGANIZATION_ID}
    ...
    def run(self):
        ...
        self.client_org.send([[round(np.mean(accuracy_list), 4), round(np.mean(accuracy_std_list), 4), round(np.mean(accuracy_99th_list), 4), round(np.mean(accuracy_90th_list), 4)],
                                 ...
                                 [round(np.mean(precision_list), 4), round(np.mean(precision_std_list), 4), round(np.mean(precision_99th_list), 4), round(np.mean(precision_90th_list), 4)],
                                 [round(np.mean(mean_time_list), 4), round(np.mean(mean_time_std_list), 4), round(np.mean(mean_time_99th_list), 4), round(np.mean(mean_time_90th_list), 4)],
