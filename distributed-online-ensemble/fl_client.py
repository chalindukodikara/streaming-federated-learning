    ...
    def run(self):
        ...
        while not self.ITERATION_FLAG:
            ...
            if self.iteration_number <= self.NUM_TIMESTAMPS:
                # edges = pd.read_csv('data/' + str(ORGANIZATION_ID) + '/' + self.dataset_name + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(self.iteration_number) + '_test_batch_edges.csv')
                # nodes = pd.read_csv('data/' + str(ORGANIZATION_ID) + '/' + self.dataset_name + '_' + str(PARTITION_SIZE) + '_' + str(PARTITION_ID) + '/' + str(self.iteration_number) + '_test_batch_nodes.csv', index_col=0)
