...
class Server:
    ...
    def __init__(self, MODEL, training_rounds, rounds, weights_path, graph_id, MAX_CONN = 2, IP= socket.gethostname(), PORT = 5000, HEADER_LENGTH = 10, iteration_number=0, transfer_learning=False, num_timestamps=0):
        ...
        while True:
            try:
                self.client_org = Client(address, authkey=b'secret password')
                break
            except:
                pass

        address = ('localhost', (7000 + ORGANIZATION_ID * 100 + 1))  # family is deduced to be 'AF_INET'
        listener = Listener(address, authkey=b'secret password')
        self.listener_org = listener.accept()
    ...
    def run(self):
        ...
        self.listener_org.send([[round(np.mean(accuracy_list), 4), round(np.mean(accuracy_std_list), 4),
                                 round(np.mean(accuracy_99th_list), 4), round(np.mean(accuracy_90th_list), 4)],
                                [round(np.mean(recall_list), 4), round(np.mean(recall_std_list), 4),
                                 round(np.mean(recall_99th_list), 4), round(np.mean(recall_90th_list), 4)],
                                [round(np.mean(auc_list), 4), round(np.mean(auc_std_list), 4),
                                 round(np.mean(auc_99th_list), 4), round(np.mean(auc_90th_list), 4)],
                                [round(np.mean(f1_list), 4), round(np.mean(f1_std_list), 4),
                                 round(np.mean(f1_99th_list), 4), round(np.mean(f1_90th_list), 4)],
                                [round(np.mean(precision_list), 4), round(np.mean(precision_std_list), 4),
                                 round(np.mean(precision_99th_list), 4), round(np.mean(precision_90th_list), 4)],
                                [round(np.mean(mean_time_list), 4), round(np.mean(mean_time_std_list), 4),
                                 round(np.mean(mean_time_99th_list), 4), round(np.mean(mean_time_90th_list), 4)],
                                [round(np.mean(total_time_list), 4), round(np.std(total_time_list), 4)]])
